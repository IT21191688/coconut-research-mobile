...
const MoldDetectionView: React.FC = () => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {/* Show button only if no mold detection result */}
        {imageUri && !moldResult && (
          <TouchableOpacity style={styles.submitButton} onPress={handleSubmit} disabled={loading}>
            {loading ? (
              <ActivityIndicator size="small" color={colors.white} />
            ) : (
              <Text style={styles.submitButtonText}>Mold Detection</Text>
            )}
          </TouchableOpacity>
        )}

        {/* Show mold detection result */}
        {moldResult && (
          <View style={styles.resultContainer}>
            <Text style={styles.resultTitle}>Mold Detection Result</Text>
            <Text style={styles.resultText}>
              {moldResult.class} (Confidence: {(moldResult.confidence * 100).toFixed(2)}%)
            </Text>
          </View>
        )}
      </ScrollView>
...
export default MoldDetectionView;
