...
const LocationWateringHistoryScreen: React.FC = () => {
  ...
  const [chartType, setChartType] = useState<'amount' | 'efficiency' | 'frequency'>('amount');
  ...
  const loadWateringHistory = async () => {
    try {
      ...
      const historyData = await getLocationSchedules(
        locationId,
        {
      ...
      Alert.alert('Error', 'Failed to load watering history. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const getChartData = () => {
    ...
    const efficiencyData = dates.map(date => {
      const daySchedules = groupedSchedules[date].filter(s => s.status === 'completed' && s.actualAmount !== undefined);
      
      if (daySchedules.length === 0) return 0;
      
      const averageEfficiency = daySchedules.reduce((sum, schedule) => {
        ...
        const efficiency = 100 - Math.abs(((actual - recommended) / recommended) * 100);
  ...
  const getSummaryStats = () => {
    ...
    const totalWaterUsed = completedSchedules.reduce((sum, s) => 
      sum + (s.actualAmount || s.recommendedAmount), 0);
    
    const averageAmount = totalWaterUsed / (completedSchedules.length || 1);
    
    const schedulesWithActualAmount = completedSchedules.filter(s => s.actualAmount !== undefined);
    const totalEfficiency = schedulesWithActualAmount.reduce((sum, s) => {
      const efficiency = 100 - Math.abs(
        ((s.actualAmount || s.recommendedAmount) - s.recommendedAmount) / s.recommendedAmount * 100
      );
      return sum + Math.max(0, Math.min(100, efficiency)); // Clamp between 0-100%
    }, 0);
    
    const averageEfficiency = schedulesWithActualAmount.length > 0 
  ...
  const chartConfig = {
    ...
    decimalPlaces: chartType === 'amount' ? 0 : chartType === 'efficiency' ? 0 : 0,
    color: (opacity = 1) => `rgba(72, 132, 244, ${opacity})`,
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <Card style={styles.chartCard}>
          <View style={styles.chartHeader}>
            ...
            <View style={styles.chartTypeButtons}>
              <TouchableOpacity 
                style={[styles.chartTypeButton, chartType === 'amount' && styles.activeChartTypeButton]}
                onPress={() => setChartType('amount')}
              >
                <Text style={[styles.chartTypeText, chartType === 'amount' && styles.activeChartTypeText]}>Amount</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={[styles.chartTypeButton, chartType === 'efficiency' && styles.activeChartTypeButton]}
                onPress={() => setChartType('efficiency')}
              >
                <Text style={[styles.chartTypeText, chartType === 'efficiency' && styles.activeChartTypeText]}>Efficiency</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={[styles.chartTypeButton, chartType === 'frequency' && styles.activeChartTypeButton]}
                onPress={() => setChartType('frequency')}
              >
                <Text style={[styles.chartTypeText, chartType === 'frequency' && styles.activeChartTypeText]}>Frequency</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          {schedules.length > 0 ? (
            <>
              <LineChart
                ...
                bezier
                style={styles.chart}
                yAxisSuffix={chartType === 'amount' ? "L" : chartType === 'efficiency' ? "%" : ""}
              />
        ...
        {/* <Button
          title="Create New Watering Schedule"
          leftIcon={<Ionicons name="add-circle-outline" size={18} color={colors.white} />}
          variant="primary"
          onPress={() => navigation.navigate('CreateSchedule', { locationId })}
